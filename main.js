...
// A:ブロックの中の変数にはブロック外からアクセスできない。ブロック内からブロック外の変数にはアクセス可能。const,letを使う。
...
//A:  ifやfor内で定義されている変数にブロック外からアクセスできる。関数の引数や関数内で定義されている変数には関数外からアクセスできない。varを使う。
...
kadai_3("数値じゃない", function (message) {
  alert(message);
});
